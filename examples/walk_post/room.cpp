    ...
    class WalkWindow final : public mxvk::VK_IOWindow {
      public:
        WalkWindow(const Arguments &args) : mxvk::VK_IOWindow(args.path, "FPS Maze Room - MXVK", args.width, args.height, args.fullscreen, args.enable_vsync), assetRoot((args.path.empty() || args.path == ".") ? std::string(WALK_ASSET_DIR) : args.path), shaderRoot(assetRoot + "/data"), modelRoot(assetRoot + "/data") {
            ...
            loadPostProcessingShaderIndex(args.shaderPath);
            setPostProcessingShaderIndex(args.shader_index);
            applyPostProcessingShaderSelection();
        ...
        void console_proc() override {
            ...
            updatePostProcessingShaderUniforms(deltaTime);
            ...
            if (!visible()) {
                ...
                if (!postProcessingShaders.empty()) {
                    printText(std::format("Post FX: {} / {}  R/T", postProcessingShaderIndex + 1, postProcessingShaders.size()), 20, 104, {180, 220, 255, 255});
                }
            }
        }
      ...
      private:
        ...
        [[nodiscard]] static std::string trimLine(std::string value) {
        ...
        [[nodiscard]] static std::string joinPath(const std::string &base, const std::string &file) {
            const std::filesystem::path filePath(file);
            if (base.empty() || filePath.is_absolute()) {
                return filePath.string();
            }
            return (std::filesystem::path(base) / filePath).string();
        }

        [[nodiscard]] static std::string resolvePostProcessingShaderEntry(const std::string &shaderPath, const std::string &entry) {
            const std::filesystem::path entryPath(entry);
            if (entryPath.is_absolute() || entryPath.extension() == ".spv") {
                return joinPath(shaderPath, entry);
            }

            std::filesystem::path spvEntry = entryPath.parent_path() / "spv" / entryPath.stem();
            spvEntry.replace_extension(".spv");
            const std::filesystem::path spvPath = std::filesystem::path(shaderPath) / spvEntry;
            if (std::filesystem::exists(spvPath)) {
                return spvPath.string();
            }

            std::filesystem::path siblingEntry = entryPath;
            siblingEntry.replace_extension(".spv");
            const std::filesystem::path siblingSpvPath = std::filesystem::path(shaderPath) / siblingEntry;
            if (std::filesystem::exists(siblingSpvPath)) {
                return siblingSpvPath.string();
            }

            return joinPath(shaderPath, entry);
        }

        void loadPostProcessingShaderIndex(const std::string &shaderPath) {
            postProcessingShaderPath = shaderPath;
            postProcessingShaders.clear();
            postProcessingShaderIndex = 0;

            if (postProcessingShaderPath.empty()) {
                setPostProcessingEnabled(false);
                return;
            }

            const std::string indexPath = joinPath(postProcessingShaderPath, "index.txt");
            std::ifstream input(indexPath);
            if (!input.is_open()) {
                throw mxvk::Exception("walk_post: failed to open post-processing shader index: " + indexPath);
            }
            ...
            while (std::getline(input, line)) {
                ...
                const std::string shaderFile = resolvePostProcessingShaderEntry(postProcessingShaderPath, entry);
                if (std::filesystem::path(shaderFile).extension() != ".spv") {
                    throw mxvk::Exception("walk_post: post-processing shader entry is not SPIR-V: " + entry);
                }
                if (!std::filesystem::exists(shaderFile)) {
                    throw mxvk::Exception("walk_post: post-processing shader listed in index.txt was not found: " + shaderFile);
                }

                postProcessingShaders.push_back(shaderFile);
            }

            if (postProcessingShaders.empty()) {
                throw mxvk::Exception("walk_post: post-processing shader index did not list any shaders: " + indexPath);
            }

            logEnv(std::format("loaded {} post-processing shader(s) from {}", postProcessingShaders.size(), indexPath));
        }

        void setPostProcessingShaderIndex(const int index) {
            if (postProcessingShaders.empty()) {
                postProcessingShaderIndex = 0;
                return;
            }

            const int shaderCount = static_cast<int>(postProcessingShaders.size());
            postProcessingShaderIndex = index % shaderCount;
            if (postProcessingShaderIndex < 0) {
                postProcessingShaderIndex += shaderCount;
            }
        }

        [[nodiscard]] const std::string &currentPostProcessingShader() const { return postProcessingShaders[static_cast<std::size_t>(postProcessingShaderIndex)]; }

        void applyPostProcessingShaderSelection() {
            if (postProcessingShaders.empty()) {
                return;
            }
            ...
            if (postProcessingSprite != nullptr) {
                ...
                logEnv(std::format("post-processing shader {} of {}: {}", postProcessingShaderIndex + 1, postProcessingShaders.size(), currentPostProcessingShader()));
                return;
            }
        ...
        void selectPostProcessingShader(const int direction) {
            if (postProcessingShaders.empty()) {
                return;
            }

            setPostProcessingShaderIndex(postProcessingShaderIndex + direction);
        ...
        void updatePostProcessingShaderUniforms(const float deltaTime) {
            if (postProcessingSprite == nullptr || postProcessingShaders.empty()) {
                return;
            }

            const auto now = std::chrono::steady_clock::now();
            const float elapsed = std::chrono::duration<float>(now - postProcessingStartTime).count();
            const float frameDelta = std::max(deltaTime, std::chrono::duration<float>(now - previousPostProcessingTime).count());
            previousPostProcessingTime = now;
            ++postProcessingFrameCount;

            float mouseX = 0.0f;
            float mouseY = 0.0f;
            const SDL_MouseButtonFlags mouseButtons = SDL_GetMouseState(&mouseX, &mouseY);
            const float mousePressed = (mouseButtons & SDL_BUTTON_LMASK) != 0U ? 1.0f : 0.0f;
            const float frameRate = frameDelta > 0.0001f ? (1.0f / frameDelta) : 0.0f;
        ...
        std::string postProcessingShaderPath{};
        std::vector<std::string> postProcessingShaders{};
        mxvk::VK_Sprite *postProcessingSprite = nullptr;
        int postProcessingShaderIndex = 0;
        uint32_t postProcessingFrameCount = 0;
        std::chrono::steady_clock::time_point postProcessingStartTime{std::chrono::steady_clock::now()};
        std::chrono::steady_clock::time_point previousPostProcessingTime{postProcessingStartTime};
