...
class SpaceRoxWindow : public mxvk::VK_Window {
  public:
    ...
    void event(SDL_Event &e) override {
        ...
        case SDL_EVENT_KEY_DOWN:
            ...
            if (e.key.key == SDLK_SPACE && state == STATE_GAMEOVER) {
                restartGame();
            }
        ...
        case SDL_EVENT_GAMEPAD_BUTTON_UP:
            handleControllerButtonUp(e.gbutton.button);
            break;
        }
    }
    ...
    void pollController() {
        ...
        Sint16 rawLX = SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFTX);
        Sint16 rawLY = SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFTY);
  ...
  private:
    ...
    void initializeResources() {
        ...
        std::array<std::uint8_t, 4 * 4 * 4> starPixels{};
        for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) {
                float dx = (x - 1.5f) / 1.5f;
                float dy = (y - 1.5f) / 1.5f;
                float dist = sqrtf(dx * dx + dy * dy);
                std::uint8_t alpha = (dist < 1.0f) ? static_cast<std::uint8_t>(255.0f * (1.0f - dist)) : 0;
                const int idx = (y * 4 + x) * 4;
                starPixels[idx + 0] = 255;
                starPixels[idx + 1] = 255;
                starPixels[idx + 2] = 255;
                starPixels[idx + 3] = alpha;
            }
        }
    ...
    void updateLaunch() {
        ...
        if (launchTimer >= launchDuration) {
            state = STATE_PLAYING;
            if (gamepad != nullptr) {
                joyRestLX = SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFTX);
                joyRestLY = SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFTY);
            }
        }
    }
