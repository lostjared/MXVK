...
namespace example {
    class HeadlessExample final : public mxvk::VK_Window {
      public:
        HeadlessExample() : mxvk::VK_Window("MXVK Headless", WIDTH, HEIGHT, false, MXVK_VALIDATION, PresentModePreference::LowLatency, RuntimeMode::Headless) {
      ...
      protected:
        ...
        void onFrameReadback(std::vector<std::uint8_t> &rgba_pixels, uint32_t width, uint32_t height) override {
            ...
            const bool pixels_available = rgba_pixels.size() == static_cast<size_t>(WIDTH) * HEIGHT * 4U;
            ...
            const bool color_matches = rgba_pixels[center] >= 30U && rgba_pixels[center] <= 34U && rgba_pixels[center + 1U] >= 62U && rgba_pixels[center + 1U] <= 66U && rgba_pixels[center + 2U] >= 126U && rgba_pixels[center + 2U] <= 130U;
            if (!color_matches) {
            ...
            std::cout << "headless example: surface-free RGBA readback verified " << width << 'x' << height << '\n';
            exit();
        }
