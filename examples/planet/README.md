...
The rain font size can be changed with `--font-size <px>`, and the font file itself can be overridden with `--font-path <file>`. Passing `--binary` switches the glyph set to `0` and `1` and uses the bundled `binary.ttf` font by default. The bundled default without `--binary` is the bold Japanese face.
