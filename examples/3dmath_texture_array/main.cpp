...
namespace {
    ...
    SurfacePtr create_frame_surface(int width, int height) {
        ...
        if (!surface) {
            throw mxvk::Exception(std::format("Failed to create 3dmath_texture_array frame surface: {}", SDL_GetError()));
        }
        return surface;
    }
    ...
    [[nodiscard]] std::string resolve_texture_path(const Arguments &args) {
        ...
        if (texture_path.empty()) {
            throw mxvk::Exception("3dmath_texture_array: pass a PNG with --filename <file.png> or --texture <file.png>");
        }
    ...
    [[nodiscard]] Texture load_texture(const std::string &path) {
        ...
        if (!loaded) {
            throw mxvk::Exception(std::format("3dmath_texture_array: failed to load PNG '{}'", path));
        }

        SurfacePtr rgba(SDL_ConvertSurface(loaded.get(), SDL_PIXELFORMAT_RGBA32));
        if (!rgba) {
            throw mxvk::Exception(std::format("3dmath_texture_array: failed to convert PNG '{}': {}", path, SDL_GetError()));
        }

        const SDL_PixelFormatDetails *format = SDL_GetPixelFormatDetails(rgba->format);
        if (format == nullptr) {
            throw mxvk::Exception(std::format("3dmath_texture_array: failed to query PNG format '{}': {}", path, SDL_GetError()));
        }
...
namespace example {
    class Math3DTextureArrayWindow : public mxvk::VK_Window {
      public:
        Math3DTextureArrayWindow(const Arguments &args, const std::string &title) : mxvk::VK_Window(title, args.width, args.height, args.fullscreen, MXVK_VALIDATION, args.enable_vsync), texture(load_texture(resolve_texture_path(args))), frame_width(args.framebuffer.width), frame_height(args.framebuffer.height), fallback_width(args.width), fallback_height(args.height) {
        ...
        void proc() override {
            ...
            const std::array<mxvk::vec4D, 8> cube_vertices = {
                mxvk::vec4D{-CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, 1.0f},
                mxvk::vec4D{CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, 1.0f},
                mxvk::vec4D{CUBE_HALF_EXTENT, CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, 1.0f},
                mxvk::vec4D{-CUBE_HALF_EXTENT, CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, 1.0f},
                mxvk::vec4D{-CUBE_HALF_EXTENT, -CUBE_HALF_EXTENT, CUBE_HALF_EXTENT, 1.0f},
            ...
            faces.reserve(GRID_CUBE_COUNT * cube_faces.size());

            for (int grid_z = -GRID_RADIUS; grid_z <= GRID_RADIUS; ++grid_z) {
                for (int grid_y = -GRID_RADIUS; grid_y <= GRID_RADIUS; ++grid_y) {
                    for (int grid_x = -GRID_RADIUS; grid_x <= GRID_RADIUS; ++grid_x) {
                        const mxvk::vec4D cube_center(static_cast<float>(grid_x) * GRID_SPACING, static_cast<float>(grid_y) * GRID_SPACING, static_cast<float>(grid_z) * GRID_SPACING, 0.0f);

                        std::array<mxvk::vec4D, 8> camera_vertices{};
                        std::array<mxvk::vec4D, 8> projected{};
                        for (std::size_t i = 0; i < cube_vertices.size(); ++i) {
                            mxvk::vec4D point = rotation.MulVec(cube_vertices[i] + cube_center);
                        ...
                        for (const auto &indices : cube_faces) {
                            const auto index0 = static_cast<std::size_t>(indices[0]);
                            const auto index1 = static_cast<std::size_t>(indices[1]);
                            const auto index2 = static_cast<std::size_t>(indices[2]);
                            const auto index3 = static_cast<std::size_t>(indices[3]);
                            const mxvk::vec4D &a = camera_vertices[index0];
                            const mxvk::vec4D &b = camera_vertices[index1];
                            const mxvk::vec4D &c = camera_vertices[index2];
                            mxvk::vec4D normal = mxvk::vec4D().Build(a, b).CrossProduct(mxvk::vec4D().Build(a, c));
                            normal.Normalize();

                            const mxvk::vec4D center = (a + b + c + camera_vertices[index3]) * 0.25f;
                            ...
                            FaceDraw face;
                            face.vertices = {{
                                {projected[index0], {0.0f, 1.0f}, camera_vertices[index0].z},
                                {projected[index1], {1.0f, 1.0f}, camera_vertices[index1].z},
                                {projected[index2], {1.0f, 0.0f}, camera_vertices[index2].z},
                                {projected[index3], {0.0f, 0.0f}, camera_vertices[index3].z},
                            }};
                            face.depth = center.z;
                            face.intensity = std::clamp(0.35f + diffuse * 0.65f, 0.0f, 1.0f);
            ...
            for (const FaceDraw &face : faces) {
                draw_textured_triangle(face.vertices[0], face.vertices[1], face.vertices[2], face.intensity);
                draw_textured_triangle(face.vertices[0], face.vertices[2], face.vertices[3], face.intensity);
            }
      ...
      private:
        ...
        float camera_distance = 8.5f;
        static constexpr int GRID_RADIUS = 1;
        static constexpr std::size_t GRID_WIDTH = static_cast<std::size_t>((GRID_RADIUS * 2) + 1);
        static constexpr std::size_t GRID_CUBE_COUNT = GRID_WIDTH * GRID_WIDTH * GRID_WIDTH;
        static constexpr float CUBE_HALF_EXTENT = 0.52f;
        static constexpr float GRID_SPACING = 1.45f;
        static constexpr float MIN_CAMERA_DISTANCE = 5.0f;
        static constexpr float MAX_CAMERA_DISTANCE = 18.0f;
        static constexpr float CAMERA_ZOOM_STEP = 0.65f;
        ...
        void ensure_framebuffer() {
            ...
            if (frame_format == nullptr) {
                throw mxvk::Exception(std::format("Failed to query 3dmath_texture_array frame format: {}", SDL_GetError()));
            }
...
int main(int argc, char **argv) {
    try {
        ...
        example::Math3DTextureArrayWindow window(args, "MXVK 3D Math Texture Array");
        window.loop();
