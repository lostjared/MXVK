...
void main() {
    ...
    vec2 uv = mirrorWrap(fragTexCoord + vec2(sin(time * 0.18), cos(time * 0.12)) * 0.025);
    ...
    vec3 glass = mix(vec3(1.0), texColor.rgb, 0.86);
    vec3 glow = vec3(1.0) * (0.015 + internal * 0.025);
    ...
    vec3 phongGlass = glass * (ambient * 0.72 + diffuse * 0.34);
    ...
    outColor = vec4(litColor, surfaceAlpha * texColor.a);
}
