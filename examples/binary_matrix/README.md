...
The digits are rendered with `SDL_ttf` into off-screen surfaces, converted into `VK_Sprite3D` batches, and drawn inside a 3D camera. A separate background sprite receives the mouse position and button state every frame so the backdrop reacts to input even though the main scene is mostly keyboard-driven.
