    ...
    void buildGeometry() {
        ...
        for (int i = 0; i < segments; ++i) {
            ...
            indices.insert(indices.end(), {
                static_cast<uint32_t>(current), static_cast<uint32_t>(current + 1), static_cast<uint32_t>(next),
                static_cast<uint32_t>(next), static_cast<uint32_t>(current + 1), static_cast<uint32_t>(next + 1),
            });
        }
...
class RawWallRenderer {
  public:
    ...
    void render(VkCommandBuffer cmd,
        ...
        // Overlap between collinear segments causes coplanar z-fighting shimmer.
        // Keep this at 0 for the procedural wall path.
        constexpr float wallOverlap = 0.0f;
        for (const WallSegment &segment : walls) {
            ...
            model = glm::scale(model, glm::vec3(length + wallOverlap * 2.0f, segment.height, 1.0f));
            vkCmdPushConstants(cmd, pipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &model);
            vkCmdDrawIndexed(cmd, indexCount_, 1, 0, 0, 0);
        }
    }
  ...
  private:
    ...
    void buildGeometry() {
        // Unit wall plane: centered at X=0, Y runs 0..1, Z=0. Quad covers -0.5..0.5 in X.
        // Keep a single surface and render with culling disabled to avoid
        // coplanar front/back duplicates that can shimmer when camera moves.
        ...
        // Front face (normal +Z)
        verts.push_back({glm::vec3(-0.5f, 0.0f, 0.0f), glm::vec2(0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)});
        verts.push_back({glm::vec3(0.5f, 0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)});
        verts.push_back({glm::vec3(0.5f, 1.0f, 0.0f), glm::vec2(1.0f, 1.0f), glm::vec3(0.0f, 0.0f, 1.0f)});
        verts.push_back({glm::vec3(-0.5f, 1.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 1.0f)});
        inds.insert(inds.end(), {0,1,2, 2,3,0});
...
class WalkWindow final : public mxvk::VK_Window {
  public:
    ...
    void onRecordCustomRendering(VkCommandBuffer cmd, uint32_t imageIndex) override {
        ...
        rawWallRenderer_.render(cmd, imageIndex, world_.walls(), view, proj, glm::vec4(0.58f, 0.58f, 0.65f, t));
  ...
  private:
    ...
    void updatePlayer(float deltaTime) {
        ...
        const float playerRadius = 0.5f;
        if (!world_.checkWallCollision(desired, playerRadius) && !world_.checkPillarCollision(desired, playerRadius)) {
            cameraPos_ = desired;
        }
