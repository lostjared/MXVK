...
vec3 preBlendColor(vec2 uv, float time_f, vec2 iResolution) {
    vec3 tex = tentBlur3(texSampler, uv, iResolution);
...
void main() {
    ...
    // Prevent division by zero if textureSize returns 0
    float aspect = (iResolution.y == 0.0) ? 1.0 : iResolution.x / iResolution.y;
    ...
    vec2 m = vec2(0.5); // Defaulted mouse to center
    vec2 ar = vec2(aspect, 1.0);
    vec3 baseCol = preBlendColor(tc, time_f, iResolution);
    ...
    vec3 rC = preBlendColor(u0 + off, time_f, iResolution);
    vec3 gC = preBlendColor(u1, time_f, iResolution);
    vec3 bC = preBlendColor(u2 - off, time_f, iResolution);
