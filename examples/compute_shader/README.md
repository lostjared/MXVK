...
Use `--filename <file>` for video playback. If `--filename` is not provided, the example falls back to `--camera <index>`. Use `--output <file>` and `--crf <value>` to control the MXWrite recording output; when omitted, the example writes `compute_shader_output.mp4` with CRF `24` into the example asset directory.
