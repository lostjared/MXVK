...
void MainWindow::setupCentralWidget() {
    ...
    consoleOutput->setStyleSheet("QPlainTextEdit { background-color: #1e1e1e; color: #39ff14; }");
...
void MainWindow::dropEvent(QDropEvent *event) {
    ...
    if (mimeData->hasUrls()) {
        ...
        if (!urls.isEmpty()) {
            ...
            if (suffix == "mxmod" || filePath.endsWith(".mxmod.z") || suffix == "obj") {
                modelLineEdit->setText(filePath);
            } else if (suffix == "tex" || suffix == "png" || suffix == "jpg" || suffix == "jpeg" || suffix == "bmp" || suffix == "mtl") {
...
void MainWindow::validatePaths() {
    ...
    bool valid = !modelPath.isEmpty();

    if (valid) {
        valid = QFileInfo(modelPath).exists();
        if (valid && !texturePath.isEmpty()) {
            valid = QFileInfo(texturePath).exists();
        }
        if (valid && !textureDir.isEmpty()) {
            valid = QDir(textureDir).exists();
        }
    }
...
QString MainWindow::resolveExecutablePath() const {
    const QString configuredPath = executablePath.trimmed();
    if (!configuredPath.isEmpty() && configuredPath != "viewer") {
        return configuredPath;
    }

    const QString appPath = QCoreApplication::applicationDirPath();
    const QStringList candidates = {
        QDir(appPath).filePath("viewer"),
        QDir(appPath).filePath("examples/viewer/viewer"),
        QDir(appPath).filePath("../examples/viewer/viewer"),
        QDir(appPath).filePath("../../examples/viewer/viewer"),
        QDir(appPath).filePath("../build/examples/viewer/viewer"),
        QDir(appPath).filePath("../../build/examples/viewer/viewer"),
        "viewer",
    };
    ...
    for (const QString &candidate : candidates) {
        ...
        if ((info.exists() && info.isExecutable()) || candidate == "viewer") {
...
void MainWindow::openModelViewer() {
    ...
    connect(process, &QProcess::errorOccurred, [this, process](QProcess::ProcessError error) {
        ...
        case QProcess::FailedToStart:
            errorMsg = "Failed to start. Is the selected viewer executable installed or configured?";
            break;
    ...
    const QString selectedExecutable = resolveExecutablePath();

    QStringList arguments;
    arguments << "--filename" << modelPath;
    if (!texturePath.isEmpty()) {
        arguments << "--texture" << texturePath;
    }
    if (!textureDir.isEmpty()) {
        arguments << "--resource_path" << textureDir;
    }
    QString resolution = resolutionCombo->currentText().split(" ").first();
    arguments << "-r" << resolution;
    QString rendererAssetPath = QCoreApplication::applicationDirPath();
    const QFileInfo selectedExecutableInfo(selectedExecutable);
    if (selectedExecutableInfo.exists()) {
...
void MainWindow::loadSettings() {
    ...
    executablePath = settings->value("executablePath", "").toString();
...
void MainWindow::showSettingsDialog() {
    ...
    QLabel *infoLabel = new QLabel(tr("Specify a custom MXVK viewer path. Leave blank to auto-detect the viewer or use viewer from PATH."), &settingsDialog);
    ...
    execPathEdit->setPlaceholderText(tr("viewer or /full/path/to/viewer"));
    ...
    QLabel *noteLabel = new QLabel(tr("Note: Leave blank for auto-detection. Use a full path if the executable is in a custom location."), &settingsDialog);
