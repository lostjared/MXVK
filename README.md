...
Current development is on version `0.35.0`. The latest release adds an optional
...
- August 25, 2026: version `0.30.0` separates native scene/post-processing
  ...
  readback source-sized, then presents an aspect-preserving fitted copy with
  an independently sized preview-only font.
...
- August 25, 2026: version `0.28.1` prefers host-cached Vulkan memory for
  ...
  to hardware encoders on its encoder thread instead of the render thread.
...
- July 2, 2026: the new `starfield` example demonstrates `VK_PointSpriteBatch` with 50,000 layered stars, per-star color/twinkle/size variation, additive blending, and a hold-`Space` warp-speed boost.
