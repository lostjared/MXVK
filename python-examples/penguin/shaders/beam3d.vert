...
void main() {
    ...
    float channel = -1.0;
    if (kind < 0.5) {
        position = ribbonPoint(incomingStart, entry + incomingDir * 0.05, along, side, 0.018, 0.0);
    } else if (kind < 1.5) {
        position = ribbonPoint(entry, exit, along, side, 0.012, 0.0);
    } else {
        ...
        position = ribbonPoint(exit + outDir * 0.006, exit + outDir * 48.0, along, side, 0.060, 0.150);
    }
