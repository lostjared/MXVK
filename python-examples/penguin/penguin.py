...
class PenguinWindow(mxvk.VK_Window):
    TITLE_TEXT = "Python Penguin"
    ...
    def __init__(self, filename: str, path: str, title: str, width: int, height: int, fullscreen: bool, enable_vsync: bool):
        ...
        try:
            self.set_clear_color(0.0, 0.0, 0.0, 0.0)
            self.set_font(str(self.data_directory / "font.ttf"), 48)

            if (filename):
                model_path = Path(filename).expanduser()
                material_path = model_path.with_suffix(".mtl")
            else:
                model_path = self.data_directory / "classic_tux.obj"
                material_path = self.data_directory / "classic_tux.mtl"

            model_vertex_shader = self.data_directory / "model.vert.spv"
            model_fragment_shader = self.data_directory / "model.frag.spv"
            gradient_fragment_shader = self.data_directory / "gradient_mix.frag.spv"

            self.model = mxvk.AbstractModel()
            self.model.load(self, str(model_path), str(material_path) if material_path.is_file() else "", str(model_path.parent), 1.0)
            self.model.set_shaders(self, str(model_vertex_shader), str(model_fragment_shader))
...
def parse_args():
    parser = argparse.ArgumentParser(description="MXVK textured Penguin Python example")
...
def main() -> int:
    ...
    window = None
    try:
        window = PenguinWindow(filename, args.path, "MXVK Python Penguin", width, height, args.fullscreen, args.enable_vsync)
        window.loop()
